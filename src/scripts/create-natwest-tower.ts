/**
 * Create a draft Etsy listing for the NatWest Tower (now Tower 42, London).
 *
 * Real height: 183m (architectural). Three-leaf cantilever plan, wf=1.0.
 * Standard 7 tower scales. Mono-colour with 13 colour options.
 *
 * Dry-run by default. Use --apply to create on Etsy.
 *
 * Usage: npx tsx src/scripts/create-natwest-tower.ts [--apply]
 */
...
// ─── Pricing (three-leaf tower, wf=1.0) ──────────────────────
// Three projecting wings give it more bulk than a slim cylinder
// but each wing is slim. Comparable to Goldin 117, Merdeka 118.
...
const TITLE = "NatWest Tower (Tower 42) – 3D Printed Skyscraper Model";

const DESCRIPTION =
  "This 3D-printed model of Tower 42 — the original NatWest Tower — captures one of the City of London’s most distinctive 1980s skyscrapers. At 183m, it was the UK’s tallest building for over a decade and remains a defining feature of the City skyline.\n\n" +
  "Designed by Richard Seifert and opened in 1981, the building’s three-leaf plan was famously said to mirror the old NatWest logo when viewed from above (something the architect always denied). The model captures that distinctive cantilevered profile with the three projecting wings extending from the central core.\n\n" +
  "A solid pick for a London skyline collection — sits naturally alongside the Gherkin, Cheesegrater and Walkie Talkie in any City of London display.";
